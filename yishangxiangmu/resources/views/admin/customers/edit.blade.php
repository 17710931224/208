...
<div class="panel panel-default">
  ...
  <div class="panel-body">
    <form action="/admin/customer/{{$results[0]->customer_id}}" class="form-horizontal" enctype="multipart/form-data" id="form-customer" method="post">
      <div class="text-right" style="margin-bottom: -28px;">
      	{{csrf_field()}}
	    {{method_field('PUT')}}
	    <button class="btn btn-sm btn-primary"><i class="fa fa-save"></i> 修改</button>
	    <a href="/admin/customer" data-toggle="tooltip" title="" class="btn btn-sm btn-default" data-original-title="取消"><i class="fa fa-reply"></i> 取消</a>
	  </div>
      <ul class="nav nav-tabs">
        ...
        <li><a data-toggle="tab" href="#tab-username">昵称</a></li>
        <li><a href="#tab-history" data-toggle="tab">历史</a></li>
        <li><a href="#tab-transaction" data-toggle="tab">交易</a></li>
        <li><a href="#tab-reward" data-toggle="tab">积分</a></li>
      ...
      <div class="tab-content">
          ...
          <div class="row">
	        <div class="col-sm-2">
	          <ul class="nav nav-pills nav-stacked" id="address">
	            ...
	            @php $addr_len = count($address_res); @endphp
				@foreach ($address_res as $key => $value)
	            <li><a href="#tab-address{{ 1+$key }}" data-toggle="tab"><i class="fa fa-minus-circle" onclick="$('#address a:first').tab('show'); $('#address a[href=\'#tab-address{{ 1+$key }}\']').parent().remove(); $('#tab-address{{ 1+$key }}').remove();"></i> 地址 {{ 1+$key }}</a></li>
				@endforeach
	            <li id="address-add">
	        ...
	        <div class="col-sm-10">
	          <div class="tab-content">
	            <div class="tab-pane active" id="tab-customer">
	              <fieldset>
	                ...
	                <div class="form-group required">
	                  ...
	                  <div class="col-sm-10"><input class="form-control" id="input-firstname" name="firstname" placeholder="名字" value="{{$results[0]->firstname}}" type="text"/></div>
	                </div>
	                <div class="form-group required">
	                  <label class="col-sm-2 control-label" for="input-lastname">姓氏</label>
	                  <div class="col-sm-10"><input class="form-control" id="input-lastname" name="lastname" value="{{$results[0]->lastname}}" placeholder="姓氏" type="text"/></div>
	                </div>
	                <div class="form-group required">
	                  <label class="col-sm-2 control-label" for="input-email">电子邮件</label>
	                  <div class="col-sm-10"><input class="form-control" id="input-email" name="email" value="{{$results[0]->email}}"  placeholder="电子邮箱地址" type="text"/></div>
	                </div>
	                <div class="form-group required">
	                  <label class="col-sm-2 control-label" for="input-mobile">手机号</label>
	                  <div class="col-sm-10"><input class="form-control" id="input-mobile" name="mobile" value="{{$results[0]->mobile}}"  placeholder="手机号" type="text"/></div>
	                </div>
	              </fieldset>
	              <fieldset>
	                <legend>密码</legend>
	                ...
	                <div class="form-group">
	                  ...
	                  <div class="col-sm-10">
	                    <select class="form-control" id="input-newsletter" name="newsletter">
	                      <option @if ($results[0]->status == "1")checked @endif value="1">启用</option>
	                      <option @if ($results[0]->status == "0")checked @endif value="0">禁用</option>
	                    </select>
	                  </div>
	                </div>
	                <div class="form-group">
	                  ...
	                  <div class="col-sm-10">
	                    <select class="form-control" id="input-status" name="status">
	                      <option  @if ($results[0]->status == "1")selected @endif value="1">启用</option>
	                      <option  @if ($results[0]->status == "0")selected @endif value="0">禁用</option>
	                    </select>
	                  </div>
	                </div>
	                <div class="form-group">
	                  ...
	                  <div class="col-sm-10">
	                    <select class="form-control" id="input-safe" name="safe">
	                      <option @if ($results[0]->status == "1")selected @endif value="1">是</option>
	                      <option @if ($results[0]->status == "0")selected @endif value="0">否</option>
	                    </select>
	                  </div>
	                </div>
	              </fieldset>
				...
				@foreach ($address_res as $key => $value)
				<div class="tab-pane" id="tab-address{{1+$key}}">
				    <div class="form-group required">
				        <label class="col-sm-2 control-label" for="input-firstname{{1+$key}}">名字</label>
				        <div class="col-sm-10"><input class="form-control" id="input-firstname{{1+$key}}" name="address[{{1+$key}}][firstname]" placeholder="名字" type="text" value="{{ $value['firstname'] }}"/></div>
				    </div>
				    <div class="form-group required">
				        <label class="col-sm-2 control-label" for="input-lastname{{1+$key}}">姓氏</label>
				        <div class="col-sm-10"><input class="form-control" id="input-lastname{{1+$key}}" name="address[{{1+$key}}][lastname]" placeholder="姓氏" type="text" value="{{ $value['lastname'] }}"/></div>
				    </div>
				    <div class="form-group required">
				        <label class="col-sm-2 control-label" for="input-address{{1+$key}}">地址</label>
				        <div class="col-sm-10"><input class="form-control" id="input-address{{1+$key}}" name="address[{{1+$key}}][address]" placeholder="地址" type="text" value="{{ $value['address'] }}"/></div>
				    </div>
				    <div class="form-group required">
				        <label class="col-sm-2 control-label" for="input-city{{1+$key}}">市（县）</label>
				        <div class="col-sm-10"><input class="form-control" id="input-city{{1+$key}}" name="address[{{1+$key}}][city]" placeholder="市（县）" type="text" value="{{ $value['city'] }}"/></div>
				    </div>
				    <div class="form-group required">
				        <label class="col-sm-2 control-label" for="input-postcode{{1+$key}}">邮政编码</label>
				        <div class="col-sm-10"><input class="form-control" id="input-postcode{{1+$key}}" name="address[{{1+$key}}][postcode]" placeholder="邮政编码" type="text" value="{{ $value['postcode'] }}"/></div>
				    </div>
				    <div class="form-group required">
				        <label class="col-sm-2 control-label" for="input-country{{1+$key}}">国家及受管轄地区（域）</label>
				        <div class="col-sm-10">
				            <select class="form-control" id="input-country{{1+$key}}" name="address[{{1+$key}}][country_id]" onchange="country(this, '{{1+$key}}', '{{$value['zone_id']}}');">
				                <option value="">--- Please Select ---</option>
				            	<?php foreach ($countries as $k => $v): ?>
								<option value="<?= $v->country_id; ?>" <?php if ($v->country_id == $value['country_id']): ?> selected="selected" <?php endif; ?>> <?= $v->name ?></option>
								<?php endforeach; ?>
				            </select>
				        </div>
				    </div>
				    <div class="form-group required">
				        <label class="col-sm-2 control-label" for="input-zone{{1+$key}}">省份/自治（州）/直辖（市）</label>
				        <div class="col-sm-10">
				        	<select class="form-control" id="input-zone{{1+$key}}" name="address[{{1+$key}}][zone_id]">
				    ...
				    <div class="form-group">
				        ...
				        <div class="col-sm-10">
				        	<label class="radio">
				        		@if ($results[0]['address_id'] == 0)
				        		<input type="radio" name="address[{{1+$key}}][default]" value="{{ 1+$key }}" checked="checked" />
	                            @else
	                            <input type="radio" name="address[{{ 1+$key }}][default]" value="{{ 1+$key }}" />
	                            @endif
	                        </label>
				        </div>
				    </div>
	    ...
	    <div class="tab-pane" id="tab-affiliate">
	        <fieldset>
	            ...
	            <div class="form-group">
	                ...
	                <div class="col-sm-10"><input class="form-control" id="input-company" name="company" placeholder="公司" type="text" value="{{$results[0]->CustomerAffiliate->company}}"/></div>
	            </div>
	            <div class="form-group">
	                <label class="col-sm-2 control-label" for="input-website">网站</label>
	                <div class="col-sm-10"><input class="form-control" id="input-website" name="website" placeholder="网站" type="text" value="{{$results[0]->CustomerAffiliate->website}}"/></div>
	            </div>
	            <div class="form-group">
	                ...
	                <div class="col-sm-10"><input class="form-control" id="input-tracking" name="tracking" placeholder="跟踪代码" type="text" value="{{$results[0]->CustomerAffiliate->tracking}}"/></div>
	            </div>
	            <div class="form-group">
	                ...
	                <div class="col-sm-10"><input class="form-control" id="input-commission" name="commission" placeholder="佣金 (%)" type="text" value="{{$results[0]->CustomerAffiliate->commission}}"/></div>
	            </div>
	        </fieldset>
	        <fieldset>
	            <legend>付款明细</legend>
	            <div class="form-group">
	                ...
	                <div class="col-sm-10"><input class="form-control" id="input-tax" name="tax" placeholder="传真编号" type="text" value="{{$results[0]->CustomerAffiliate->tax}}"/></div>
	            </div>
	            <div class="form-group">
	                ...
	                <div class="col-sm-10">
	                    <div class="radio">
	                        <label>
	                            <input checked="checked" name="payment" type="radio" @if ($results[0]->CustomerAffiliate->payment == "cheque")checked @endif value="cheque"/>
	                            Cheque
	                        </label>
	                    </div>
	                    <div class="radio">
	                        <label>
	                            <input name="payment" type="radio" @if ($results[0]->CustomerAffiliate->payment == "paypal")checked @endif value="paypal"/>
	                            PayPal
	                        </label>
	                    </div>
	                    <div class="radio">
	                        <label>
	                            <input name="payment" type="radio" @if ($results[0]->CustomerAffiliate->payment == "bank")checked @endif value="bank"/>
	                            银行转帐
	                        </label>
	                    </div>
	                </div>
	            ...
	            <div class="payment" id="payment-cheque" style="display: block;">
	                <div class="form-group required">
	                    ...
	                    <div class="col-sm-10"><input class="form-control" id="input-cheque" name="cheque" placeholder="检查收款人姓名" type="text" value="{{$results[0]->CustomerAffiliate->cheque}}"/></div>
	                </div>
	            </div>
	            <div class="payment" id="payment-paypal" style="display: none;">
	                <div class="form-group required">
	                    ...
	                    <div class="col-sm-10"><input class="form-control" id="input-paypal" name="paypal" placeholder="PayPal电子邮件帐户" type="text" value="{{$results[0]->CustomerAffiliate->paypal}}"/></div>
	                </div>
	            </div>
	            <div class="payment" id="payment-bank" style="display: none;">
	                <div class="form-group">
	                    <label class="col-sm-2 control-label" for="input-bank-name">银行名称</label>
	                    <div class="col-sm-10"><input class="form-control" id="input-bank-name" name="bank_name" placeholder="银行名称" type="text" value="{{$results[0]->CustomerAffiliate->bank_name}}"/></div>
	                </div>
	                <div class="form-group">
	                    <label class="col-sm-2 control-label" for="input-bank-branch-number">银行ABA/BSB 编号（分支编号）</label>
	                    <div class="col-sm-10"><input class="form-control" id="input-bank-branch-number" name="bank_branch_number" placeholder="ABA/BSB 编号（分支编号）" type="text" value="{{$results[0]->CustomerAffiliate->bank_branch_number}}"/></div>
	                </div>
	                <div class="form-group">
	                    <label class="col-sm-2 control-label" for="input-bank-swift-code">银行SWIFT 码</label>
	                    <div class="col-sm-10"><input class="form-control" id="input-bank-swift-code" name="bank_swift_code" placeholder="SWIFT 码" type="text" value="{{$results[0]->CustomerAffiliate->bank_swift_code}}"/></div>
	                </div>
	                <div class="form-group required">
	                    <label class="col-sm-2 control-label" for="input-bank-account-name">银行账户名称</label>
	                    <div class="col-sm-10"><input class="form-control" id="input-bank-account-name" name="bank_account_name" placeholder="账户名称" type="text" value="{{$results[0]->CustomerAffiliate->bank_account_name}}"/></div>
	                </div>
	                <div class="form-group required">
	                    <label class="col-sm-2 control-label" for="input-bank-account-number">银行账户编号</label>
	                    <div class="col-sm-10"><input class="form-control" id="input-bank-account-number" name="bank_account_number" placeholder="账户编号" type="text" value="{{$results[0]->CustomerAffiliate->bank_account_number}}"/></div>
	                </div>
	            </div>
	            <div class="form-group">
	                ...
	                <div class="col-sm-10">
	                    <select class="form-control" id="input-affiliate" name="affiliate_status">
	                        <option @if ($results[0]->CustomerAffiliate->affiliate_status == "1")selected @endif value="1">启用</option>
	                        <option @if ($results[0]->CustomerAffiliate->affiliate_status == "0")selected @endif value="0">禁用</option>
	                    </select>
	                </div>
	            </div>
	        </fieldset>
		...
		<div class="tab-pane" id="tab-history">
		  <fieldset>
		    ...
		    <div id="history">
		      <div class="table-responsive">
		          ...
		          <thead>
		            <tr>
		              <td class="text-left">加入日期</td>
		              <td class="text-left">评论</td>
		            </tr>
		          </thead>
		          <tbody>
		            @foreach ($history_res as $k => $v)
					<tr>
		              <td class="text-left">{{@date('m/d/Y', strtotime($v->created_at))}}</td>
		              <td class="text-left">{{$v->comment}}</td>
		            </tr>
		            @endforeach
		          </tbody>
		        </table>
		  ...
		  <fieldset>
		    <legend>添加历史</legend>
		    <div class="form-group">
		      <label class="col-sm-2 control-label" for="input-comment">评论</label>
		      <div class="col-sm-10"><textarea class="form-control" id="input-comment" name="comment" placeholder="评论" rows="8"></textarea></div>
		    </div>
		  </fieldset>
		  <div class="text-right">
		    <a href="javascript:void(0)" class="btn btn-primary" data-loading-text="加载中..." id="button-history">
		      <i class="fa fa-plus-circle"></i>
		      添加历史
		    </a>
		  </div>
		...
		<div class="tab-pane" id="tab-transaction">
		        ...
		        <table class="table table-bordered table-hover">
		          <thead>
		            <tr>
		              ...
		              <td class="text-right">交易量</td>
		            </tr>
		          </thead>
		          <tbody>
		          	<tr>
						<td class="text-left">15/10/2018</td>
					...
					<tr>
						<td class="text-left">09/10/2018</td>
		  ...
		  <fieldset>
		    ...
		    <div class="form-group">
		      ...
		      <div class="col-sm-10"><input class="form-control" id="input-transaction-description" name="description" placeholder="Description" type="text" value=""/></div>
		    </div>
		    <div class="form-group">
		      <label class="col-sm-2 control-label" for="input-amount">交易量</label>
		      <div class="col-sm-10"><input class="form-control" id="input-amount" name="amount" placeholder="交易量" type="text" value=""/></div>
		    </div>
		  </fieldset>
		  <div class="text-right">
		    <button class="btn btn-primary" data-loading-text="Loading..." id="button-transaction" type="button">
		      <i class="fa fa-plus-circle"></i>
		      添加交易
		    </button>
		  </div>
		...
		<div class="tab-pane" id="tab-reward">
		  ...
		  <fieldset>
		    <legend>添加积分</legend>
		    <div class="form-group">
		      <label class="col-sm-2 control-label" for="input-reward-description">描述</label>
		      <div class="col-sm-10">
		      	<input class="form-control" id="input-reward-description" name="description" placeholder="描述" type="text" value=""/>
		      </div>
		    </div>
		    <div class="form-group">
		      <label class="col-sm-2 control-label" for="input-points"><span data-original-title="Use minus to remove points" data-toggle="tooltip" title="">分数</span></label>
		      <div class="col-sm-10">
		      	<input class="form-control" id="input-points" name="points" placeholder="分数" type="text" value=""/>
		      </div>
		    </div>
		  </fieldset>
		  <div class="text-right">
		    <button class="btn btn-primary" data-loading-text="Loading..." id="button-reward" type="button">
		      <i class="fa fa-plus-circle"></i>
		      添加积分
		    </button>
		  </div>
	    ...
	    <div class="tab-pane" id="tab-username">
			<legend>昵称</legend>
			<div class="form-group">
                <label class="col-sm-2 control-label" for="input-username">昵称</label>
                <div class="col-sm-10"><input class="form-control" id="input-username" name="username" placeholder="昵称" type="text" value=""></div>
            </div>
	    </div>
                ...
                <table class="table table-bordered table-hover">
                  ...
                  <tbody>
                  	...
                  	@if (count($ips) > 0)
	                  	...
	                  	<tr>
					        <td class="text-left">
					        	<a href="http://www.geoiptool.com/en/?IP={{$v->ip}}" target="_blank">{{$v->ip}}</a>
					        </td>
					        <td class="text-right">
					        	<a href="/admin/customer?user_token=VtAnMxbpj4hQaQwLvFNHPf1vsYKkeNfQ&filter_ip={{$v->ip}}" target="_blank">{{count($v)}}</a>
					        </td>
					        <td class="text-left">{{@date('m/d/Y', strtotime($v->created_at))}}</td>
	        			</tr>
	        			@endforeach
...
<script type="text/javascript">
	...
	$('#button-history').click(function (){
		...
		//  $('tbody tr').html("<td class='text-left'></td><td class='text-left'></td>")
		if(comments.length != 0){
			$.post('/admin/customer_histories', {"_token":"{{csrf_token()}}","_method":"DELETE",comments:comments,customer_id:{{$results[0]->customer_id}}, }, function(data){
			  	// SCRIPT_NAME
			  	str_replace($_SERVER['SCRIPT_NAME'], "", $_SERVER['REQUEST_URI'])
					url = "{{@$_SERVER['SCRIPT_NAME']}}";
				    
				    url += encodeURIComponent("#tab-history");

				    location.replace(url+"{{@str_replace($_SERVER['SCRIPT_NAME'], "", @$_SERVER['REQUEST_URI'])}}");
...
/*
  ...
  $('#history').load('/customer/history&user_token=CeO560kvOtNEZjScl8KtYhES0PiSTw2Y&customer_id=1');
...
/*
  ...
  $('#transaction').load('/customer/transaction&user_token=CeO560kvOtNEZjScl8KtYhES0PiSTw2Y&customer_id=1');
...
/*
    ...
    $.ajax({
      ...
      success: function(json) {
        ...
        if (json['success']) {
          ...
          $('#reward').load('/customer/reward&user_token=CeO560kvOtNEZjScl8KtYhES0PiSTw2Y&customer_id=1');
  ...
  $('#ip').load('/customer/ip&user_token=CeO560kvOtNEZjScl8KtYhES0PiSTw2Y&customer_id=1');
