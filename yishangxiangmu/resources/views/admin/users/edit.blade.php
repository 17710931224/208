...
<div class="container-fluid">
  <div class="panel panel-default">
    ...
    <div class="panel-body">
      <form action="/admin/user/{{$results[0]->user_id}}" class="form-horizontal" enctype="multipart/form-data" id="form-user" method="post">
        <div class="form-group required">
          <label class="col-sm-2 control-label" for="input-username">使用者</label>
          <div class="col-sm-10"><input class="form-control" id="input-username" name="username" placeholder="使用者" type="text" value="{{$results[0]->username}}"/></div>
        </div>
        <div class="form-group">
          ...
          <div class="col-sm-10">
            <select class="form-control" id="input-user-group" name="user_group_id">
              <option @if($results[0]->user_group_id == 1) selected @endif value="1">管理员</option>
              <option @if($results[0]->user_group_id == 10) selected @endif value="10">Demonstration</option>
            </select>
          </div>
        </div>
        <div class="form-group required">
          <label class="col-sm-2 control-label" for="input-firstname">名字</label>
          <div class="col-sm-10"><input class="form-control" id="input-firstname" name="firstname" placeholder="名字" type="text" value="{{$results[0]->firstname}}"/></div>
        </div>
        <div class="form-group required">
          <label class="col-sm-2 control-label" for="input-lastname">姓氏</label>
          <div class="col-sm-10"><input class="form-control" id="input-lastname" name="lastname" placeholder="姓氏" type="text" value="{{$results[0]->lastname}}"/></div>
        </div>
        <div class="form-group required">
          <label class="col-sm-2 control-label" for="input-email">电子邮件</label>
          <div class="col-sm-10"><input class="form-control" id="input-email" name="email" placeholder="电子邮件" type="text" value="{{$results[0]->email}}"/></div>
        </div>
        <div class="form-group">
          <label class="col-sm-2 control-label" for="input-image">头像</label>
          <div class="col-sm-10">
            <img alt="" data-placeholder="{{$results[0]->avatar}}" id="avatar" src="{{$results[0]->avatar}}" width="100" title=""/>
            <br />图片正常大小: 100 * 100
        ...
        <div class="form-group">
          ...
          <div class="col-sm-10">
            <select class="form-control" id="input-status" name="status">
              <option @if($results[0]->user_group_id == 0) selected @endif value="0">禁用</option>
              <option @if($results[0]->user_group_id == 1) selected @endif value="1">启用</option>
            </select>
          </div>
        </div>
        {{method_field('PUT')}}
